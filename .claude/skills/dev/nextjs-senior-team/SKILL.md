...
## Order Check — לפני כל ייעוץ

לפני שהצוות פותח פה, @arch בודק:
1. האם השאלה מתאימה לשלב הנוכחי? (תשתית / עיצוב / לוגיקה / תוכן)
2. האם כל מה שהשאלה הזו מסתמכת עליו כבר קיים בפרויקט?
3. האם יש החלטות קודמות שסותרות את מה שנשאל?

אם @arch מגלה סתירה או קפיצת שלב → HUMAN מיד, ללא ייעוץ.
...
## Blind Voting — כשיש מחלוקת בין חברי הצוות

כאשר שאלה מקבלת תשובות מנוגדות מחברי הצוות, עבור ל-Blind Voting:

**כלל:** כל חבר מצביע לפני שרואה עמדות אחרים.

```
@[role]: [🟢 APPROVE / 🟡 CONDITIONAL / 🔴 BLOCK]
Confidence: [HIGH / MEDIUM / LOW]
נימוק: ...
סיכון שאני עלול לפספס: ...
```

**Gate Rules:**
- אחד 🔴 → HUMAN
- 2+ LOW confidence → החזר שאלת הבהרה
- Deadlock (ראיות חזקות לשני כיוונים) → HUMAN עם הסבר:
  ```
  ⚠️ DEADLOCK — נדרשת החלטת ערכים
  @arch: [עמדה] כי [א, ב, ג]
  @perf: [עמדה הפוכה] כי [ד, ה, ו]
  שניהם צודקים — זו שאלה עסקית, לא טכנית:
  [מה לבחור?]
  ```
...
אחרי כל ייעוץ — @arch בודק:
האם שאלה דומה הגיעה 3+ פעמים?
כן → זה לא שאלה נקודתית. זה סימפטום של בעיה ארכיטקטורלית.
פעולה: צור Linear issue "בעיה ארכיטקטורלית חוזרת: [נושא]"
